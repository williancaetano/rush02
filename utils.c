...
void print_hundred(char number, t_dictionary* dictionary)
{
    ...
    current = dictionary;
    while (current != NULL) {
        if(ft_atoi(charToString(number)) == current->nb){
...
void print_100(char* number, t_dictionary* dictionary)
{
    ...
    if(nb > 99)
    {
        ...
        nb = nb - (ft_atoi(charToString(number[0])) * 100);
    }
    if(nb == 0)
        return;
    if(nb >= 20)
    {
        print_number(ft_atoi(charToString(number[1])) * 10, dictionary);
        if((nb % 10) != 0)
            print_number(ft_atoi(charToString(number[2])), dictionary);
    }
    if(nb < 20)
    {
